...
class Player:
    ...
    def teleport(self, area=None, grid_x=None, grid_y=None):
        """
        Teleport the player to a specific area and/or coordinates.
        
        Args:
            area: The area to teleport to. If None, stay in current area.
            grid_x: The x-coordinate within the area's grid. If None, use current or center.
            grid_y: The y-coordinate within the area's grid. If None, use current or center.
            
        Returns:
            bool: True if teleport was successful, False otherwise.
        """
        # If no area specified, teleport within current area
        if area is None:
            if not self.current_area:
        ...
        # Get current grid position if we're staying in the same area
        if area == self.current_area and (grid_x is None or grid_y is None):
            current_x, current_y, _ = self.get_grid_position()
            grid_x = grid_x if grid_x is not None else current_x
            grid_y = grid_y if grid_y is not None else current_y
        else:
            # Use center of area if no coordinates specified
            grid_x = grid_x if grid_x is not None else area.grid_width // 2
            grid_y = grid_y if grid_y is not None else area.grid_length // 2
        
        # Validate coordinates are within area bounds
        if not (0 <= grid_x < area.grid_width and 0 <= grid_y < area.grid_length):
            print(f"Invalid coordinates ({grid_x}, {grid_y}) for {area.name}.")
            print(f"Valid range: (0-{area.grid_width-1}, 0-{area.grid_length-1})")
            return False
        
        # If player was hidden, they're no longer hidden after teleporting
        if self.is_hidden:
            self.unhide()
        
        # Teleport to the specified area and coordinates
        if area == self.current_area:
            # Just update coordinates within the same area
            self.coordinates.x = area.coordinates.x + grid_x
            self.coordinates.y = area.coordinates.y + grid_y
            print(f"You teleport to coordinates ({grid_x}, {grid_y}) in {area.name}.")
            self.look_around()
        else:
            # Change area and update coordinates
