...
class UseItemBehavior(Behavior):
    ...
    def perform(self, game):
        ...
        if should_pickup and hasattr(self.npc, 'items') and hasattr(self.npc, 'add_item'):
            # Check if the item is still in the location (might have been picked up by another NPC)
            if self.item in self.npc.location.items:
                # Check if the item has been picked up this turn
                if hasattr(game, '_items_picked_up_this_turn') and self.item in game._items_picked_up_this_turn:
                    # Item has already been picked up by another NPC this turn
                    return f"{self.npc.name} looks for the {self.item.name}, but someone else already took it."
                
                # Try to add the item to NPC's inventory - add_item will check if it's a valid Item subclass
                if self.npc.add_item(self.item):
                    # Only remove from location if successfully added to inventory
                    self.npc.location.items.remove(self.item)
                    
                    # Track that this item has been picked up this turn
                    if not hasattr(game, '_items_picked_up_this_turn'):
                        game._items_picked_up_this_turn = []
                    game._items_picked_up_this_turn.append(self.item)
                    ...
                    # If add_item failed (not a valid Item subclass), just interact with it
            ...
            if potential_soil:
                ...
                if self.item in self.npc.location.items:
                    ...
                    if self.npc.add_item(self.item):
                        ...
                        # Not a valid Item subclass
                        return f"{self.npc.name} examines the {self.item.name} but doesn't know how to plant it."
                elif hasattr(self.npc, 'items') and self.item in self.npc.items:
                    self.npc.items.remove(self.item)
    ...
    def _pickup_weapon(self, game):
        ...
        if not has_better_weapon:
            # Pick up the weapon
            if self.item in self.npc.location.items:
                # Check if the weapon has been picked up this turn
                if hasattr(game, '_items_picked_up_this_turn') and self.item in game._items_picked_up_this_turn:
                    # Weapon has already been picked up by another NPC this turn
                    return f"{self.npc.name} looks for the {self.item.name}, but someone else already took it."
