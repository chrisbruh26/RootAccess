...
class GridArea:
    ...
    def get_full_description(self, player=None):
        ...
        if player and hasattr(player, 'x') and hasattr(player, 'y'):
            nearby_objects = self.get_objects_near_player(player, radius=5)  # Increased radius to show more objects
            if nearby_objects:
                desc += "\nNearby:\n"
                for obj, x, y in nearby_objects:
                    if obj != player:  # Don't include the player in the list
                        # Calculate Manhattan distance (number of steps needed)
                        distance = abs(player.x - x) + abs(player.y - y)
                        direction = self._get_direction(player.x, player.y, x, y)
                        desc += f"- {obj.name} ({distance} steps to the {direction})\n"
        
        # Add items with locations
        if self.items:
            desc += f"\nItems in the area:\n"
            for item in self.items:
                if hasattr(item, 'x') and hasattr(item, 'y') and player and hasattr(player, 'x') and hasattr(player, 'y'):
                    # Calculate Manhattan distance
                    distance = abs(player.x - item.x) + abs(player.y - item.y)
                    direction = self._get_direction(player.x, player.y, item.x, item.y)
                    desc += f"- {item.name} ({distance} steps to the {direction})\n"
                else:
                    desc += f"- {item.name}\n"
        
        # Add NPCs with locations
        if self.npcs:
            desc += f"\nNPCs in the area:\n"
            for npc in self.npcs:
                if hasattr(npc, 'x') and hasattr(npc, 'y') and player and hasattr(player, 'x') and hasattr(player, 'y'):
                    # Calculate Manhattan distance
                    distance = abs(player.x - npc.x) + abs(player.y - npc.y)
                    direction = self._get_direction(player.x, player.y, npc.x, npc.y)
                    desc += f"- {npc.name} ({distance} steps to the {direction})\n"
                else:
                    desc += f"- {npc.name}\n"
        
        # Add objects with locations
        if self.objects:
            desc += f"\nObjects in the area:\n"
            for obj in self.objects:
                if hasattr(obj, 'x') and hasattr(obj, 'y') and player and hasattr(player, 'x') and hasattr(player, 'y'):
                    # Calculate Manhattan distance
                    distance = abs(player.x - obj.x) + abs(player.y - obj.y)
                    direction = self._get_direction(player.x, player.y, obj.x, obj.y)
                    desc += f"- {obj.name} ({distance} steps to the {direction})\n"
                else:
                    desc += f"- {obj.name}\n"
    ...
    def _get_direction(self, from_x, from_y, to_x, to_y):
        ...
        if abs(dx) > abs(dy):
            # East-West is primary direction
            if dx > 0:
                primary_direction = "east"
            else:
                primary_direction = "west"
            
            # Add north-south component if significant
            if abs(dy) > 0:
                if dy > 0:
                    secondary_direction = "south"
                else:
                    secondary_direction = "north"
                direction = f"{secondary_direction}{primary_direction}"
            else:
                direction = primary_direction
        else:
            # North-South is primary direction
            if dy > 0:
                primary_direction = "south"
            else:
                primary_direction = "north"
            
            # Add east-west component if significant
            if abs(dx) > 0:
                if dx > 0:
