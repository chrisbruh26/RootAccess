...
class MessageCoordinator:
    ...
    def summarize_npc_actions(self, npc_actions):
        """
        Summarize all NPC actions into a single natural-language message, limited to 5 NPCs.
        ...
        """
        # Limit the number of NPCs to 5
        npc_actions = npc_actions[:5]
        ...
        def format_npc_list(npcs):
            """Format a list of NPC names, limiting to 3 and summarizing the rest."""
            if len(npcs) <= 3:
                return ", ".join(npcs)
            else:
                return f"{', '.join(npcs[:3])}, and {len(npcs) - 3} others"

        # Combat actions
        if action_categories["combat"]:
            combat_npcs = [npc for npc, _ in action_categories["combat"]]
            summary_parts.append(f"{format_npc_list(combat_npcs)} engaged in combat.")

        # Gardening actions
        if action_categories["gardening"]:
            gardening_npcs = [npc for npc, _ in action_categories["gardening"]]
            summary_parts.append(f"{format_npc_list(gardening_npcs)} tended to plants.")

        # Interaction actions
        if action_categories["interaction"]:
            interaction_npcs = [npc for npc, _ in action_categories["interaction"]]
            summary_parts.append(f"{format_npc_list(interaction_npcs)} interacted with objects.")

        # Talking actions
        if action_categories["talking"]:
            talking_npcs = [npc for npc, _ in action_categories["talking"]]
            summary_parts.append(f"{format_npc_list(talking_npcs)} had conversations.")

        # Idle actions
        if action_categories["idle"]:
            idle_npcs = [npc for npc, _ in action_categories["idle"]]
            summary_parts.append(f"{format_npc_list(idle_npcs)} stood around idly.")

        # Other actions - replace with more specific actions using get_random_npc_action
        if action_categories["other"]:
            from RA_main import get_random_npc_action
            other_npcs = [npc for npc, _ in action_categories["other"]]
            
            # If we have 3 or fewer NPCs, give each one a specific action
            if len(other_npcs) <= 3:
                for npc in other_npcs:
                    action = get_random_npc_action()
                    # For single NPC, use the action as is (already in singular form)
                    summary_parts.append(f"{npc} {action}")
            else:
                # For larger groups, combine them with a random action
                action = get_random_npc_action()
                
                # Fix grammar for plural subjects by converting verbs to plural form
                ...
                # Apply grammar fixes
                plural_action = action
                for singular, plural in singular_endings.items():
                    if singular in plural_action:
                        plural_action = plural_action.replace(singular, plural)
                        break
                
                summary_parts.append(f"{format_npc_list(other_npcs)} {plural_action}")
