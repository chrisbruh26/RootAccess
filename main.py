...
class Game:
    ...
    def create_world(self):
        """Create the game world using the world loader."""
        # Create a world loader instance
        world_loader = WorldLoader(self)
        
        # Load world data from JSON file
        world_data_path = os.path.join(os.path.dirname(__file__), "data", "world_data.json")
        
        # Check if the world data file exists
        if os.path.exists(world_data_path):
            success, message = world_loader.load_world_data(world_data_path)
            if not success:
                print(f"Error loading world data: {message}")
                # Fall back to hardcoded world creation if loading fails
                self._create_hardcoded_world()
        else:
            print(f"World data file not found at {world_data_path}")
            # Fall back to hardcoded world creation if file doesn't exist
            self._create_hardcoded_world()
            
    def _create_hardcoded_world(self):
        """Create a hardcoded world as a fallback if JSON loading fails."""
        print("Creating hardcoded world...")
