...
def group_hazard_results(hazard, results):
    ...
    # Process resisted members
    if resisted_members:
        if len(resisted_members) == 1:
            messages.append(f"The {gang_name} member {resisted_members[0]} resists the {hazard.name} effect!")
        elif len(resisted_members) <= 3:
            member_list = ", ".join(resisted_members)
            messages.append(f"The {gang_name} members {member_list} resist the {hazard.name} effect!")
        else:
            sample = resisted_members[:2]
            messages.append(f"The {gang_name} members {sample[0]}, {sample[1]} and {len(resisted_members) - 2} others resist the {hazard.name} effect!")
