        ...
        while self.is_running:
            ...
            if cmd_entry:
                ...
                for member in gang_members:
                    ...
                    if any(effect.name == "hallucinations" for effect in member.active_effects):
                        hallucinating_members.append(member)
                    else:
                        normal_attackers.append(member)

                # Add hallucinating members messages to the message manager
                if hallucinating_members:
                    ...
                    if len(hallucinating_members) == len(gang_members):
                        undetected_msg = random.choice(NPC_REACTIONS.get("possible_hallucinations", {}).get("player_undetected", ["None of the gang members see you because they are too distracted from the hallucinations."]))
                        self.npc_message_manager.add_message(undetected_msg)
                    else:
                        for member in hallucinating_members:
                            self.npc_message_manager.add_message(f"The {gang_name} member {member.name} is so high that they don't see you.")

                # Add attack messages for non-hallucinating members to the message manager
                for attacker in normal_attackers:
                    attack_result = attacker.attack_player(self.player)
                    if attack_result:
                        self.npc_message_manager.add_message(attack_result)
