...
class GangMember(NPC):
    ...
    def update_behavior(self, game):
        ...
        if hasattr(game, 'player') and self.location == game.player.current_area and self.is_alive:
            ...
            # Only return a combined result if both parts have content
            if behavior_result and attack_result:
                ...
                # Return None if both are None to avoid printing "None" in the game
                return None
        return behavior_result
...
class NPCMessageManager:
    ...
    def _combine_npc_actions(self):
        ...
        # For NPCs with multiple actions, we'll only use the first action
        # This is the simplest solution to prevent duplicate NPC names in action messages
        for npc_name, actions in self.npc_actions.items():
