...
class Game:
    ...
    def game_loop(self):
        ...
        while self.is_running:
            ...
            if cmd_entry:
                # Start a new turn for NPC message tracking BEFORE processing the command
                # This ensures NPC actions are reset only once per player turn
                ...
                for obj in list(self.player.current_area.objects):
                    if isinstance(obj, StaticHazard) and obj.active:
                        ...
                        if hazard_result:
                            # Add hazard results to the message manager
                            self.npc_message_manager.add_message(hazard_result)
                            
                            # Add significant hazard events to notifications
                            if "affected:" in hazard_result:
                ...
                # This ensures each NPC is only processed once per turn
                ...
                for npc in self.player.current_area.npcs:
                    ...
                    if hasattr(npc, 'update_behavior'):
                        ...
                        if behavior_result:
                            self.npc_message_manager.add_message(behavior_result)
                            
                            # Add significant NPC behaviors to notifications
                            if any(keyword in behavior_result.lower() for keyword in 
                                  ["planting", "harvesting", "watering", "gives you", "gift", 
                                   "offers you", "item", "defeated", "died"]):
                                self.player.notification_manager.add_notification(
                                    behavior_result, 
                                    category="npc", 
                                    importance=2
                ...
                # to prevent overwhelming the player with messages
                ...
                for member in gang_members:
                    ...
                    # (this includes attacks, friendly interactions, etc.)
                    interaction_result = member.attack_player(self.player)
                    
                    if interaction_result:
                        self.npc_message_manager.add_message(interaction_result)
                        
                        # Add significant interactions to notifications
                        if any(keyword in interaction_result.lower() for keyword in 
                              ["gives you", "gift", "offers you", "insists you take", 
                               "item", "defeated", "died"]):
                            self.player.notification_manager.add_notification(
                                interaction_result, 
                                category="item" if "item" in interaction_result.lower() else "npc", 
                                importance=2
                            )
                    ...
                    if death_message:
                        # Death messages are important, print directly
                        print(death_message)
                        break
                ...
                # Get summarized NPC messages
                npc_summary = self.npc_message_manager.get_summary(clear=True)
                
                # Filter out messages that have been added to notifications
                filtered_summary = npc_summary
                
                # Check if there are any NPC messages that match notification criteria
                notification_keywords = [
                    "planting", "harvesting", "watering", "gives you", "gift", 
                    "offers you", "item", "defeated", "died", "affected:"
                ]
                
                # If the summary contains notification keywords, don't print it
                # The player will see these messages when they check notifications
                if npc_summary and any(keyword in npc_summary.lower() for keyword in notification_keywords):
                    # Don't print the summary, it will be in notifications
                    pass
                elif npc_summary:
                    # Print other NPC messages that aren't in notifications
                    print(npc_summary)
            else:
