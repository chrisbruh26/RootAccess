...
class NPCBehaviorCoordinator:
    ...
    def _get_behavior_type(self, behavior):
        """
        Determine the behavior type of an NPC's current behavior.

        Args:
            behavior: The current behavior object of the NPC.

        Returns:
            A string representing the behavior type.
        """
        from npc_behavior import BehaviorType, IdleBehavior, TalkBehavior, FightBehavior, UseItemBehavior

        if isinstance(behavior, IdleBehavior):
            return BehaviorType.IDLE.name.lower()
        elif isinstance(behavior, TalkBehavior):
            return BehaviorType.TALK.name.lower()
        elif isinstance(behavior, FightBehavior):
            return BehaviorType.FIGHT.name.lower()
            ...
            if hasattr(behavior, 'item'):
                ...
                if any(keyword in item_name for keyword in ["seed", "water", "fertilizer", "plant"]):
                    return BehaviorType.GARDENING.name.lower()
            return BehaviorType.USE_ITEM.name.lower()
        else:
            return "unknown"
