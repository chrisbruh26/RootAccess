...
from message_system import MessageManager, MessageCategory, MessagePriority
from npc_behavior import NPCMessageManager, BehaviorManager
...
def integrate_message_systems(game):
    """
    Integrate the improved message systems into the game.
    
    Args:
        game: The main game instance
        
    Returns:
        message_coordinator: The configured MessageCoordinator instance
    """
    # Step 1: Initialize the main message manager (you likely already have this)
    ...
    # Step 3: Initialize or get the notification manager from the player
    notification_manager = None
    if hasattr(game, 'player') and hasattr(game.player, 'notification_manager'):
        notification_manager = game.player.notification_manager
    elif hasattr(game, 'player'):
        # Create a notification manager for the player if it doesn't exist
        from notification_system import NotificationManager
        game.player.notification_manager = NotificationManager()
        notification_manager = game.player.notification_manager
    
    # Step 4: Create the message coordinator to connect all systems
    ...
    # Step 5: Store the coordinator in the game instance for easy access
...
def update_game_turn(game):
    ...
    if hasattr(game, 'message_coordinator'):
        ...
        # Check for notification reminders (only once)
        if hasattr(game, 'player') and hasattr(game.player, 'notification_manager'):
            notification_reminder = game.player.notification_manager.new_turn()
            if notification_reminder:
                game.message_coordinator.process_system_message(
...
def display_npc_summary(game):
    """
    Display the NPC action summary at the end of the turn.
    ...
    # Get and display the NPC summary
    summary = game.message_coordinator.get_npc_summary()
    if summary:
        # Add the summary to the message manager
...
class Game:
    def __init__(self):
        ...
        # Initialize notification manager for the player if it doesn't exist
        from notification_system import NotificationManager
        if not hasattr(self.player, 'notification_manager'):
            self.player.notification_manager = NotificationManager()
        ...
        self.message_coordinator = MessageCoordinator(
            ...
            notification_manager=self.player.notification_manager
        )
        self.commands = {
    ...
    def game_loop(self):
        ...
        while self.is_running:
            ...
            if hasattr(self, 'message_coordinator'):
                # Use the coordinator to reset all message systems
                self.message_coordinator.new_turn()
            else:
                # Fall back to just resetting the message manager
            ...
            if cmd_entry:
                ...
                if output:
                    # Add player command output to message system using the coordinator if available
                    if hasattr(self, 'message_coordinator'):
                        ...
                        # Fall back to direct message manager
                ...
                for obj in list(self.player.current_area.objects):
                    if isinstance(obj, StaticHazard) and obj.active:
                        ...
                        if hazard_result:
                            # Add hazard results to message system using the coordinator if available
                            if hasattr(self, 'message_coordinator'):
                ...
                for npc in self.player.current_area.npcs:
                    ...
                    if hasattr(npc, 'update_behavior'):
                        ...
                        if behavior_result:
                            ...
                            # Add to message system using the coordinator if available
                            if hasattr(self, 'message_coordinator'):
                                self.message_coordinator.process_npc_message(behavior_result, npc=npc)
                            else:
                                # Fall back to direct message manager
                                self.message_manager.add_message(
                                    behavior_result, 
                ...
                for member in gang_members:
                    ...
                    if interaction_result:
                        ...
                        # Add to message system using the coordinator if available
                        if hasattr(self, 'message_coordinator'):
                            should_show = self.message_coordinator.process_npc_message(
                                interaction_result, 
                                npc=member
                            )
                        else:
            ...
            # Display NPC summary if using the coordinator
            if hasattr(self, 'message_coordinator'):
                # Get and display the NPC summary
                summary = self.message_coordinator.get_npc_summary()
                # Only display summary if user settings allow showing hazard messages
                show_hazard_generic = self.message_manager.should_show_category(MessageCategory.HAZARD_GENERIC)
                show_hazard_specific = self.message_manager.should_show_category(MessageCategory.HAZARD_SPECIFIC)
                if summary and (show_hazard_generic or show_hazard_specific):
                    print(summary)
            ...
            message_summary = self.message_manager.get_message_summary(
                categories=[
                    ...
                    MessageCategory.NPC_SUMMARY  # Add NPC_SUMMARY to displayed categories
                ]
            )
    ...
    def cmd_message_settings(self, args):
        ...
        Categories:
          ...
          npc-summary - Summary of NPC actions
        ...
        Presets:
          quiet - Minimal NPC messages (only important ones)
          chatty - More NPC messages (see more of what's happening)
          verbose - Show most NPC messages (good for debugging)
        ...
        Examples:
          ...
          message-settings presets chatty - Apply the chatty preset (more NPC messages)
        """
        ...
        categories = {
            ...
            # Removed npc-hazard category as per user request
            "npc-summary": MessageCategory.NPC_SUMMARY,
            
            # Other categories
            # Removed hazard category as per user request
            "hazard-generic": MessageCategory.HAZARD_GENERIC,
            "hazard-specific": MessageCategory.HAZARD_SPECIFIC,
        ...
        presets = {
            "quiet": {
                # Show only important NPC actions
                MessageCategory.NPC_MINOR: (True, False, 0.9, 5),  # 10% rate, 5 turn cooldown
                MessageCategory.NPC_IDLE: (False, False, 0.95, 10),  # 5% rate, 10 turn cooldown
                MessageCategory.NPC_MOVEMENT: (True, False, 0.9, 8),  # 10% rate, 8 turn cooldown
                MessageCategory.NPC_INTERACTION: (True, False, 0.8, 4),  # 20% rate, 4 turn cooldown
                MessageCategory.NPC_TALK: (True, False, 0.8, 3),  # 20% rate, 3 turn cooldown
                MessageCategory.NPC_GIFT: (True, True, 0.2, 1),  # 80% rate, 1 turn cooldown
                MessageCategory.NPC_HAZARD: (True, False, 0.5, 2),  # 50% rate, 2 turn cooldown
                MessageCategory.NPC_SUMMARY: (True, False, 0, 0),  # 100% rate, no cooldown
                MessageCategory.HAZARD_EFFECT: (True, False, 0.9, 5),  # 10% rate, 5 turn cooldown
                MessageCategory.AMBIENT: (False, False, 0.95, 8),  # 5% rate, 8 turn cooldown
                MessageCategory.TRIVIAL: (False, False, 0.98, 15),  # 2% rate, 15 turn cooldown
            },
            "chatty": {
                # Show more NPC actions
                MessageCategory.NPC_MINOR: (True, False, 0.5, 2),  # 50% rate, 2 turn cooldown
                MessageCategory.NPC_IDLE: (True, False, 0.7, 3),  # 30% rate, 3 turn cooldown
                MessageCategory.NPC_MOVEMENT: (True, False, 0.6, 2),  # 40% rate, 2 turn cooldown
                MessageCategory.NPC_INTERACTION: (True, False, 0.4, 1),  # 60% rate, 1 turn cooldown
                MessageCategory.NPC_TALK: (True, False, 0.3, 1),  # 70% rate, 1 turn cooldown
                MessageCategory.NPC_GIFT: (True, True, 0.1, 0),  # 90% rate, no cooldown
                MessageCategory.NPC_HAZARD: (True, False, 0.2, 1),  # 80% rate, 1 turn cooldown
                MessageCategory.NPC_SUMMARY: (True, False, 0, 0),  # 100% rate, no cooldown
                MessageCategory.HAZARD_EFFECT: (True, False, 0.7, 3),  # 30% rate, 3 turn cooldown
                MessageCategory.AMBIENT: (True, False, 0.7, 3),  # 30% rate, 3 turn cooldown
                MessageCategory.TRIVIAL: (True, False, 0.8, 5),  # 20% rate, 5 turn cooldown
            },
            "verbose": {
                # Show almost all NPC actions (good for debugging)
                MessageCategory.NPC_MINOR: (True, False, 0.2, 1),  # 80% rate, 1 turn cooldown
                MessageCategory.NPC_IDLE: (True, False, 0.3, 2),  # 70% rate, 2 turn cooldown
                MessageCategory.NPC_MOVEMENT: (True, False, 0.2, 1),  # 80% rate, 1 turn cooldown
                MessageCategory.NPC_INTERACTION: (True, False, 0.1, 0),  # 90% rate, no cooldown
                MessageCategory.NPC_TALK: (True, False, 0.1, 0),  # 90% rate, no cooldown
                MessageCategory.NPC_GIFT: (True, True, 0, 0),  # 100% rate, no cooldown
                MessageCategory.NPC_HAZARD: (True, False, 0, 0),  # 100% rate, no cooldown
                MessageCategory.NPC_SUMMARY: (True, False, 0, 0),  # 100% rate, no cooldown
                MessageCategory.HAZARD_EFFECT: (True, False, 0.3, 1),  # 70% rate, 1 turn cooldown
                MessageCategory.AMBIENT: (True, False, 0.3, 1),  # 70% rate, 1 turn cooldown
                MessageCategory.TRIVIAL: (True, False, 0.5, 2),  # 50% rate, 2 turn cooldown
            },
            "balanced": {
                # Default balanced settings
                MessageCategory.NPC_MINOR: (True, False, 0.7, 3),  # 30% rate, 3 turn cooldown
                MessageCategory.NPC_IDLE: (True, False, 0.9, 5),  # 10% rate, 5 turn cooldown
                MessageCategory.NPC_MOVEMENT: (True, False, 0.8, 4),  # 20% rate, 4 turn cooldown
                MessageCategory.NPC_INTERACTION: (True, False, 0.6, 3),  # 40% rate, 3 turn cooldown
                MessageCategory.NPC_TALK: (True, False, 0.5, 2),  # 50% rate, 2 turn cooldown
                MessageCategory.NPC_GIFT: (False, True, 0.2, 1),  # 80% rate, 1 turn cooldown
                MessageCategory.NPC_HAZARD: (True, False, 0.3, 2),  # 70% rate, 2 turn cooldown
                MessageCategory.NPC_SUMMARY: (True, False, 0, 0),  # 100% rate, no cooldown
                MessageCategory.HAZARD_EFFECT: (True, False, 0.9, 5),  # 10% rate, 5 turn cooldown
                MessageCategory.AMBIENT: (True, False, 0.8, 4),  # 20% rate, 4 turn cooldown
                MessageCategory.TRIVIAL: (False, False, 0.95, 10),  # 5% rate, 10 turn cooldown
            }
        }
        
        # Handle no arguments - show current settings
        if not args:
            settings = []
            settings.append("Message Display Settings:")
            settings.append("=====================")
            settings.append("Use 'message-settings presets [preset_name]' to quickly apply a preset:")
            settings.append("  quiet - Minimal NPC messages (only important ones)")
            settings.append("  chatty - More NPC messages (see more of what's happening)")
            settings.append("  verbose - Show most NPC messages (good for debugging)")
            settings.append("  balanced - Balanced settings (default)")
            settings.append("")
            ...
            for name, category in categories.items():
                if category is None:  # Skip special categories
                    continue
                    
                settings.append(f"  {name}:")
                settings.append(f"    show: {self.message_manager.should_show_category(category)}")
                settings.append(f"    notify: {self.message_manager.should_notify_category(category)}")
        ...
        if args[0].lower() == "presets" and len(args) >= 2:
            preset_name = args[1].lower()
            if preset_name not in presets:
                valid_presets = ", ".join(sorted(presets.keys()))
                return f"Unknown preset: {preset_name}. Valid presets: {valid_presets}"
            
            # Apply the preset
            preset = presets[preset_name]
            for category, settings in preset.items():
                show, notify, throttle, cooldown = settings
                self.message_manager.configure_category(
                    category, 
                    show_directly=show, 
                    add_to_notifications=notify, 
                    throttle_rate=throttle, 
                    cooldown=cooldown
                )
            
            return f"Applied the '{preset_name}' message settings preset."
            
        # Handle command with arguments for individual settings
        if len(args) < 3:
        ...
        for category in target_categories:
            if setting_name == "show":
                ...
                if value.lower() in ["on", "true", "yes", "1"]:
                    self.message_manager.set_category_notify(category, True)
                elif value.lower() in ["off", "false", "no", "0"]:
                    self.message_manager.set_category_notify(category, False)
                else:
                    return f"Invalid value for 'notify': {value}. Use 'on' or 'off'."
    ...
    def cmd_behavior_settings(self, args):
        ...
        Usage: 
          ...
          behavior-settings npcs [on/off] - Enable or disable all NPC behaviors
        ...
        Examples:
          ...
          behavior-settings all frequency 0 - Set all behavior frequencies to 0%
          behavior-settings npcs off - Disable all NPC behaviors completely
          behavior-settings npcs on - Enable NPC behaviors
        """
        ...
        if not args:
            ...
            # Show global NPC enabled status
            npcs_status = "enabled" if behavior_settings.npcs_enabled else "disabled"
            settings.append(f"  NPCs: {npcs_status}")
        ...
        # Handle special case for enabling/disabling all NPCs
        if args[0].lower() == "npcs" and len(args) == 2:
            value = args[1].lower()
            if value == "on" or value == "true" or value == "1":
                behavior_settings.npcs_enabled = True
                return "NPCs enabled. They will now perform behaviors."
            elif value == "off" or value == "false" or value == "0":
                behavior_settings.npcs_enabled = False
                return "NPCs disabled. They will not perform any behaviors."
            else:
                return "Invalid value. Use 'on' or 'off'."
        
        # Regular behavior settings
        if len(args) != 3:
            return "Invalid arguments. Usage: behavior-settings [behavior] [setting] [value] or behavior-settings npcs [on/off]"
