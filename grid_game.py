...
class GridGame:
    ...
    def __init__(self):
        ...
        self.commands = {
            ...
            'find': {'handler': self.cmd_find, 'category': 'interaction'},
            'locate': {'handler': self.cmd_find, 'category': 'interaction'},
    ...
    def cmd_find(self, args):
        """Find an item, NPC, or object in the current area and provide directions to it."""
        if not self.player.current_area:
            return "You're not in any area."
        
        if not args:
            return "Find what? Specify what you're looking for."
        
        search_term = " ".join(args).lower()
        
        # Search for items
        items = [item for item in self.player.current_area.items if search_term in item.name.lower()]
        
        # Search for NPCs
        npcs = [npc for npc in self.player.current_area.npcs if search_term in npc.name.lower()]
        
        # Search for objects
        objects = [obj for obj in self.player.current_area.objects if search_term in obj.name.lower()]
        ...
        for item in items:
            if hasattr(item, 'x') and hasattr(item, 'y'):
                direction, distance = self.player._get_direction_and_distance(item.x, item.y)
                results.append(f"Item: {item.name} - {distance} steps to the {direction}")
        
        # Process NPCs
        for npc in npcs:
            if hasattr(npc, 'x') and hasattr(npc, 'y'):
                direction, distance = self.player._get_direction_and_distance(npc.x, npc.y)
                results.append(f"NPC: {npc.name} - {distance} steps to the {direction}")
        
        # Process objects
        for obj in objects:
            if hasattr(obj, 'x') and hasattr(obj, 'y'):
                direction, distance = self.player._get_direction_and_distance(obj.x, obj.y)
                results.append(f"Object: {obj.name} - {distance} steps to the {direction}")
        
        if results:
            return "Found the following:\n" + "\n".join(results)
        else:
            return f"Couldn't find anything matching '{search_term}' in this area."
    ...
    def cmd_help(self, args):
        ...
        # Add special notes for new commands
        help_text += "\nSpecial commands:\n"
        help_text += "  find/locate [item/npc/object] - Get directions to something in the area\n"
