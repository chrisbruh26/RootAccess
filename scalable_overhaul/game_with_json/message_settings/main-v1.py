            ...
            if game and hasattr(game, 'message_manager'):
                ...
                for npc in self.npcs:
                    ...
                    if hasattr(npc, 'is_alive') and not npc.is_alive:
                        npc_desc += " (dead)"
                    elif hasattr(npc, 'health'):
                        # Check if NPC has max_health attribute, otherwise use a default value
                        max_health = getattr(npc, 'max_health', 100)
                        health_percent = int((npc.health / max_health) * 100)
                        npc_desc += f" (health: {health_percent}%)"
...
class Game:
    ...
    def cmd_message_settings(self, args):
        ...
        if category_name not in category_map:
            return f"Unknown category: {category_name}. Valid categories: npc, hazard, ambient, trivial, all"
        
        category = category_map[category_name]
        categories = [category] if category else list(category_map.values())[:-1]  # All except 'all'
