...
class NPC:
    ...
    def add_item(self, item):
        """Add an item to NPC's inventory, checking if it's a valid Item or subclass."""
        # Import Item class from RA_main to check if item is a subclass
        from RA_main import Item
        
        # Check if the item is an instance of Item or any of its subclasses
        if isinstance(item, Item):
            self.items.append(item)
            return True
        else:
            # For debugging - this should rarely happen in normal gameplay
            print(f"Warning: {self.name} tried to add a non-Item object to inventory: {item}")
            return False
