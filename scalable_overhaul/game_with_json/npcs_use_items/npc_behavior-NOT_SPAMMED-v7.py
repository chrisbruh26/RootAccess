...
class NPCMessageManager:
    ...
    def new_turn(self):
        ...
        # Always reset NPC actions for the new turn - this is critical
        # to ensure we're properly tracking one action per NPC per turn
        self.npc_actions = {}
    ...
    def add_message(self, message):
        ...
        if npc_name:
            ...
            # Strict limit: only one action per NPC per turn (except for attacks)
            if self.npc_actions[npc_name] and action_type != "attack":
                # Skip this action if NPC already has an action this turn
                # (except for attacks, which are always important)
                return None
                
            # For detection status messages, be even more restrictive
            if action_type in ["unnoticed", "looking_away", "distracted"]:
                # Only show detection status every few turns (1 in 5 chance)
                # and only if we haven't shown any detection status for this NPC recently
    ...
    def _should_skip_message(self, message):
        ...
        if ("doesn't notice you" in message or "doesn't see you" in message or 
            ...
            "fails to notice you" in message or "walks past your hiding spot" in message):
            # Only show a tiny percentage (1 in 20 chance) - we'll summarize these later
            # This is a significant reduction from before
    ...
    def _create_summary(self):
        ...
        # Only show "unaware" messages occasionally (1 in 3 turns) unless we have very few messages
        if unaware_members and (len(summary_parts) < 2 or random.random() < 0.3) and len(summary_parts) < max_summary_parts:
