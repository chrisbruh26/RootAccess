...
class MessageCategory(enum.Enum):
    ...
    HAZARD_GENERIC = 17  # Generic hazard feedback (affected, resisted)
    HAZARD_SPECIFIC = 18  # Specific hazard effect descriptions (hallucinations, etc.)
...
class MessageManager:
    ...
    def __init__(self, game):
        ...
        # Initialize notification importance and categories dictionaries
        self.notification_importance = {}
        self.notification_categories = {}
        self.category_keywords = defaultdict(list)  # Keywords associated with each category
        self.category_cooldowns = defaultdict(int)  # Cooldown periods for each category
        ...
        self.display_settings = {
            ...
            MessageCategory.NPC_GIFT: (False, True, 0.2),  # Show 80% of the time
        ...
        # Add new categories for hazard messages
        self.display_settings[MessageCategory.HAZARD_GENERIC] = (True, True, 0.5)  # Show 50% of the time, allow notifications
        self.display_settings[MessageCategory.HAZARD_SPECIFIC] = (True, True, 0.7)  # Show 70% of the time, allow notifications
        
        # Configure notification importance for new categories
        self.notification_importance[MessageCategory.HAZARD_GENERIC] = 3  # Medium importance
        self.notification_importance[MessageCategory.HAZARD_SPECIFIC] = 3  # Medium importance
        
        # Configure notification categories for new categories
        self.notification_categories[MessageCategory.HAZARD_GENERIC] = "hazard-generic"
        self.notification_categories[MessageCategory.HAZARD_SPECIFIC] = "hazard-specific"
        
        # Add keywords for new categories
        self.category_keywords[MessageCategory.HAZARD_GENERIC] = [
            "are affected", "is affected", "resisted the effect", "resisted the effects", "resists the effect", "resists the effects"
        ]
        self.category_keywords[MessageCategory.HAZARD_SPECIFIC] = [
            "hallucination", "poison", "radiation", "electric", "see the walls melting", "starts seeing things", "feels dizzy"
        ]
        
        # Add cooldowns for new categories
        self.category_cooldowns[MessageCategory.HAZARD_GENERIC] = 3  # Show every 3 turns at most
        self.category_cooldowns[MessageCategory.HAZARD_SPECIFIC] = 5  # Show every 5 turns at most
    ...
    # Helper methods for message settings command
    def should_show_category(self, category):
        """Check if a category should be shown directly."""
        if category not in self.display_settings:
            return False
        return self.display_settings[category][0]
    
    def should_notify_category(self, category):
        """Check if a category should be added to notifications."""
        if category not in self.display_settings:
            return False
        return self.display_settings[category][1]
    
    def get_category_rate(self, category):
        """Get the display rate for a category (as a percentage)."""
        if category not in self.display_settings:
            return 0
        throttle_rate = self.display_settings[category][2]
        # Convert throttle rate to display percentage (throttle rate is the chance of NOT showing)
        return int((1 - throttle_rate) * 100)
    
    def get_category_cooldown(self, category):
        """Get the cooldown period for a category."""
        return self.category_cooldowns.get(category, 0)
    
    def set_category_show(self, category, show):
        """Set whether a category should be shown directly."""
        if category in self.display_settings:
            current = self.display_settings[category]
            self.display_settings[category] = (show, current[1], current[2])
    
    def set_category_notify(self, category, notify):
        """Set whether a category should be added to notifications."""
        if category in self.display_settings:
            current = self.display_settings[category]
            self.display_settings[category] = (current[0], notify, current[2])
    
    def set_category_rate(self, category, rate_percent):
        """Set the display rate for a category (as a percentage)."""
        if category in self.display_settings:
            # Convert percentage to throttle rate (throttle rate is the chance of NOT showing)
            throttle_rate = 1 - (rate_percent / 100)
            current = self.display_settings[category]
            self.display_settings[category] = (current[0], current[1], throttle_rate)
    
    def set_category_cooldown(self, category, cooldown):
