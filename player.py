...
class GridPlayer:
    ...
    def take_item(self, item_name):
        ...
        if hasattr(item, 'x') and hasattr(item, 'y') and not self._is_near_coordinates(item.x, item.y):
            ...
            direction, distance = self._get_direction_and_distance(item.x, item.y)
            return False, f"The {item_name} is {distance} steps to the {direction}."
    ...
    def interact_with_object(self, object_name):
        ...
        if hasattr(obj, 'x') and hasattr(obj, 'y') and not self._is_near_coordinates(obj.x, obj.y):
            # Calculate direction and distance
            direction, distance = self._get_direction_and_distance(obj.x, obj.y)
            return False, f"The {object_name} is {distance} steps to the {direction}."
    ...
    def _get_direction_and_distance(self, target_x, target_y):
        """Get the direction and distance from the player to the target coordinates."""
        dx = target_x - self.x
        dy = target_y - self.y
