...
- Notification Settings: Control which messages generate notifications

In-Game Commands:
---------------
message-settings                           # Show current settings
message-settings [category] show [on/off]  # Show/hide a category
message-settings [category] rate [0-100]   # Set display rate percentage
message-settings [category] cooldown [n]   # Set cooldown period in turns
...
class MessageManager:
    ...
    def __init__(self, game):
        ...
        for category, (show, notify, rate) in self.display_settings.items():
            self.show_directly[category] = show
            self.add_to_notifications[category] = notify
    ...
    def _process_message(self, message):
        ...
        if add_to_notifications and hasattr(self.game, 'player') and hasattr(self.game.player, 'notification_manager'):
            # Check if this is a notification reminder to prevent recursive notifications
            if "You have" in message.text and "unread notification" in message.text:
                # Skip adding notification reminders as notifications
                pass
            else:
