...
class NPCMessageManager:
    ...
    def add_message(self, message):
        ...
        if npc_name:
            ...
            if action_type in ["hallucination", "friendly"]:
                # Check if we already have any effect messages for any NPC
                has_effect_message = False
                for existing_npc, existing_actions in self.npc_actions.items():
                    for existing_action_type, _ in existing_actions:
                        if existing_action_type in ["hallucination", "friendly"]:
            ...
            # For gift messages, be extremely restrictive to prevent duplicates
            if action_type == "gift":
                # Check if this NPC already has a gift action
                if npc_name in self.npc_actions:
                    for existing_action_type, _ in self.npc_actions[npc_name]:
                        if existing_action_type == "gift":
                            # Already has a gift action, skip this one
                            return None
                
                # Check if the message contains an item name that's already been mentioned in another gift message
                        ...
                        for existing_action_type, existing_message in existing_actions:
                            if existing_action_type == "gift":
                                ...
                                if existing_item and existing_item == item_name:
                                    # Same item already mentioned in another gift message, skip this one
                                    return None
    ...
    def _extract_npc_info(self, message):
        ...
        if "member" in message:
            ...
            if member_idx >= 0 and member_idx < len(parts):
                ...
                elif "gives you" in message or "gift" in message or "insists you take" in message or "presses" in message or "offers you" in message:
                    action_type = "gift"
...
class BehaviorManager:
    ...
    def choose_next_behavior(self, game):
        ...
        if isinstance(self.npc, GangMember):
            behavior_weights = [0.1, 0.1, 0.7, 0.1]  # 20% idle, 20% talk, 30% fight, 30% use item
            ...
            if hasattr(game, 'player') and hasattr(game.player, 'detected_by') and self.npc.gang in game.player.detected_by:
                behavior_weights = [0.1, 0.1, 0.7, 0.1]  # 10% idle, 10% talk, 50% fight, 30% use item
        ...
        if items:
            ...
            if has_weapons or has_healing or has_seeds or has_hazards or has_gardening_tools:
                # Keep item interaction high regardless of NPC count to maintain interesting interactions
                # NPCs should interact with items freely, regardless of population
                boost_factor = 0.3  # Full boost for all NPCs
                
                # Increase UseItemBehavior weight by taking from other behaviors
                behavior_weights = [w * (1 - boost_factor) for w in behavior_weights]  # Reduce all weights
                behavior_weights[3] += boost_factor  # Add boost to UseItemBehavior
                ...
                if has_seeds:
                    ...
                    if has_soil:
                        ...
                        # NPCs should interact with seeds and soil freely, regardless of population
                        soil_boost_factor = 0.2  # Full boost for all NPCs
                        
                        # Apply the boost
                        behavior_weights = [w * (1 - soil_boost_factor) for w in behavior_weights]
                        behavior_weights[3] += soil_boost_factor
