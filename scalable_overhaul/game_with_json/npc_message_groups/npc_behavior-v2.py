...
def group_hazard_results(hazard, results):
    ...
    # Process hallucinating members - only show if there are 3 or fewer total messages
    if hallucinating_members and len(messages) < 3:
        # Limit to just one hallucination message even if multiple members are affected
        if random.random() < 0.5:  # 50% chance to show hallucination message
        ...
        elif random.random() < 0.3:  # Even lower chance (30%) for plural hallucination messages
