...
def update_game_turn(game):
    ...
    if hasattr(game, 'message_coordinator'):
        ...
        # Check for notification reminders - only do this once
        if hasattr(game, 'player') and hasattr(game.player, 'notification_manager'):
...
class Game:
    ...
    def game_loop(self):
        ...
        while self.is_running:
            ...
            # Add notification indicator with count if there are unread notifications
            if unread_count > 0:
            ...
            if cmd_entry:
                ...
                # Handle notification-worthy events directly
                if self.player.health < 50:
                    self.player.notification_manager.add_notification(
                        "Your health is low! Find a way to heal.",
                        category=MessageCategory.NOTIFICATION,
                        priority=MessagePriority.HIGH
                    )
    ...
    def cmd_notifications(self, args):
        ...
        # Read notifications - this will mark them as read automatically
        notifications_text, remaining = self.player.notification_manager.read_notifications(count, category)
        
        # Automatically clear non-important notifications after viewing
        self.player.notification_manager.clear_non_important_notifications()
        
        # Return the notifications text
    ...
    def cmd_behavior_settings(self, args):
        ...
        if not args:
            ...
            behaviors = {
                ...
                "gardening": BehaviorType.GARDENING,
...
class NotificationManager:
    def __init__(self, max_notifications=50, reminder_frequency=5):
        ...
        self.unread_count = 0  # Track unread notifications
        self.important_notifications = []  # Separate list for important notifications

    def add_notification(self, text, category=None, priority=None, important=False):
        ...
        if important:
            self.important_notifications.append(notification)
        
        return f"Notification added: {text}"

    def read_notifications(self, count=None, category=None):
        """Read notifications, marking them as read."""
        ...
        if count:
            filtered_notifications = filtered_notifications[:count]
        
        self.unread_count = sum(1 for n in self.notifications if not n["read"])
        return "\n".join(n["text"] for n in filtered_notifications), len(filtered_notifications)

    def clear_notifications(self):
        """Clear all notifications, including important ones."""
        self.notifications.clear()
        self.important_notifications.clear()
        self.unread_count = 0
        return "All notifications cleared."

    def clear_non_important_notifications(self):
        """Clear all non-important notifications."""
        self.notifications = [n for n in self.notifications if n in self.important_notifications]
        self.unread_count = sum(1 for n in self.notifications if not n["read"])
