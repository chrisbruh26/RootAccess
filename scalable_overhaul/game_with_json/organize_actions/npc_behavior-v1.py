...
class GangMember(NPC):
    ...
    def apply_hazard_effect(self, hazard):
        ...
        # Import the get_random_npc_action function
...
class BehaviorManager:
    ...
    def update(self, game):
        ...
        # Import the get_random_npc_action function for interesting actions
        from RA_main import get_random_npc_action
...
def format_npc_action(npcs, action_template):
    ...
    verb = "sound" if is_plural else "sounds", # something singular always "sounds" not "sound", recheck default list
...
# Load NPC actions from the JSON file
def load_npc_actions(filepath):
    with open(filepath, 'r') as file:
        return json.load(file)

# Select a random NPC action based on the number of NPCs
def get_random_npc_action(category, npc_count, actions):
    """
    Get a random NPC action from the specified category.

    Args:
        category (str): The category of the action (e.g., "idle", "talk").
        npc_count (int): The number of NPCs involved.
        actions (dict): The dictionary of actions loaded from the JSON file.

    Returns:
        str: A random action string.
    """
    if category not in actions:
        return None

    # Determine whether to use singular or plural actions
    key = "plural" if npc_count > 1 else "singular"
    if key in actions[category]:
        return random.choice(actions[category][key])
    return None
