...
class NPCMessageManager:
        ...
        if "member" in message:
            ...
            if member_idx >= 0 and member_idx < len(parts):
                ...
                if "attacks you" in message or "damage" in message:
                    ...
                    action_type = "gift"
                elif "standing around" in message or "idle" in message:
                    action_type = "idle"
                elif "talks to" in message:
                    action_type = "talk"
                elif "interacts with" in message or "uses" in message:
                    action_type = "interact"
                elif "doesn't notice you" in message or "doesn't see you" in message:
                    action_type = "unaware"
    ...
    def _create_summary(self):
        ...
        for message in self.message_buffer:
            ...
            # Awareness messages (noticed or not)
            elif "doesn't notice you" in message or "doesn't see you" in message or "spots you" in message:
        ...
        elif len(summary_parts) < 3 and other_messages:
    ...
    def _combine_npc_actions(self):
        ...
        for npc_name, actions in self.npc_actions.items():
            # Skip NPCs with only one action - these are handled elsewhere
            if len(actions) <= 1:
                continue
            ...
            if "idle" in action_types and "gift" in action_types:
                gift_message = next((msg for type, msg in actions if type == "gift"), "")
            ...
            # Special case: idle + talk
                ...
                talk_message = next((msg for type, msg in actions if type == "talk"), "")
            ...
            # Special case: idle + interact
                ...
                interact_message = next((msg for type, msg in actions if type == "interact"), "")
            ...
            # Default case: just list the actions
                ...
                for action_type, message in actions:
                    if action_type == "idle":
                        action_descriptions.append("standing around")
                    elif action_type == "talk":
                        if "talks to" in message:
                            target = message.split("talks to")[1].strip().rstrip(".")
                            action_descriptions.append(f"talking to {target}")
                        else:
                            action_descriptions.append("talking")
                    elif action_type == "interact":
                        if "interacts with" in message:
                            item = message.split("interacts with")[1].strip().rstrip(".")
                            action_descriptions.append(f"using {item}")
                        else:
                            action_descriptions.append("interacting with objects")
                    elif action_type == "hallucination":
                        action_descriptions.append("hallucinating")
                    elif action_type == "gift":
                        action_descriptions.append("giving away items")
                    elif action_type == "friendly":
                        action_descriptions.append("being unusually friendly")
                
                if action_descriptions:
                    if len(action_descriptions) == 2:
...
class UseItemBehavior(Behavior):
    ...
    def perform(self, game):
        # Check if the item has special interaction logic
        ...
        if hasattr(self.item, 'health_restore'):
            ...
            if self.item not in self.npc.items and hasattr(self.npc, 'add_item'):
...
class BehaviorManager:
    ...
    def choose_next_behavior(self, game):
        ...
        if isinstance(self.npc, GangMember):
            behavior_weights = [0.2, 0.2, 0.3, 0.3]  # 20% idle, 20% talk, 30% fight, 30% use item
            ...
            if any(effect.name == "hallucinations" for effect in self.npc.active_effects):
                behavior_weights = [0.3, 0.2, 0.1, 0.4]  # 30% idle, 20% talk, 10% fight, 40% use item
        ...
        if items:
            ...
            has_healing = any(hasattr(item, 'healing') for item in items)
            has_seeds = any(hasattr(item, 'type') and item.type == 'seed' for item in items)
            ...
            if has_weapons or has_healing or has_seeds or has_hazards:
                # Increase UseItemBehavior weight by taking from other behaviors
                behavior_weights = [w * 0.8 for w in behavior_weights]  # Reduce all weights by 20%
                behavior_weights[3] += 0.2  # Add 20% to UseItemBehavior
