...
def fix_plural_verb_agreement(text):
    """
    Fix verb agreement for plural subjects.
    
    Args:
        text: The text to fix
        
    Returns:
        Text with corrected verb agreement for plural subjects
    """
    ...
    for singular, plural in singular_endings.items():
        if singular in text:
            text = text.replace(singular, plural)
            break
...
def convert_to_natural_language(summary, active_gangs):
    ...
    if gardening_lines:
        if paragraph_parts:
            # Fix grammar for plural subjects if needed
            line = gardening_lines[0].lower()
            if " and " in line or "," in line:
                # Fix plural verb agreement
                line = fix_plural_verb_agreement(line)
            paragraph_parts.append(random.choice(connectors) + line)
    ...
    if interaction_lines:
        if paragraph_parts:
            # Fix grammar for plural subjects if needed
            line = interaction_lines[0].lower()
            if " and " in line or "," in line:
                # Fix plural verb agreement
                line = fix_plural_verb_agreement(line)
            paragraph_parts.append(random.choice(connectors) + line)
    ...
    for i, line in enumerate(remaining_lines[:3]):  # Limit to 3 more lines
        if paragraph_parts:
            ...
            if " and " in line_text or "," in line_text:
                # Fix plural verb agreement
                line_text = fix_plural_verb_agreement(line_text)
            paragraph_parts.append(random.choice(connectors) + line_text)
