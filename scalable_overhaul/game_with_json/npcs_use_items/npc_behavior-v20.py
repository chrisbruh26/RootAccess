...
class BehaviorManager:
    ...
    def choose_next_behavior(self, game):
        ...
        if items:
            ...
            # If there are special items, increase item interaction chance, but with limits
            if has_weapons or has_healing or has_seeds or has_hazards or has_gardening_tools:
                # Count the number of NPCs in the area
                npc_count = len([npc for npc in self.npc.location.npcs if npc.is_alive])
                
                        # Keep this high regardless of NPC count to maintain interesting interactions
                # The more NPCs in the area, the less likely each one should be to interact with items
                # This prevents all NPCs from rushing to pick up the same items
                if npc_count > 1:
                    # Scale down the item interaction boost based on NPC count
                    # More NPCs = less boost for each NPC
                    boost_factor = min(0.3, 0.3 / (npc_count * 0.5))  # Reduces as NPC count increases
                else:
                    boost_factor = 0.3  # Full boost for a single NPC
                ...
                if has_seeds:
                    ...
                    if has_soil:
                        # Further increase UseItemBehavior weight for seed planting, but with limits
                        # Count the number of NPCs in the area again
                        npc_count = len([npc for npc in self.npc.location.npcs if npc.is_alive])
                        
                        # Scale the boost based on NPC count
                        if npc_count > 1:
                            soil_boost_factor = min(0.2, 0.2 / (npc_count * 0.5))
                        else:
                            soil_boost_factor = 0.2  # Full boost for a single NPC
