...
def display_npc_summary(game):
    ...
    if summary:
        # Get active gangs for a more specific header
        active_gangs = getattr(game.message_coordinator, 'active_gangs', set())
        
        # Create a header based on active gangs
        if active_gangs and len(active_gangs) == 1:
            # Single gang
            gang_name = next(iter(active_gangs))
            header = f"NPC ACTIVITY SUMMARY - {gang_name.upper()}:"
        elif active_gangs and len(active_gangs) > 1:
            # Multiple gangs
            header = "NPC ACTIVITY SUMMARY - MULTIPLE GANGS:"
        else:
            # Default header
            header = "NPC ACTIVITY SUMMARY:"
        
        # Format the summary with the appropriate header
        formatted_summary = f"{header}\n{summary}"
        
        # Add the summary to the message manager with HIGH priority to ensure it's displayed
...
class Game:
    ...
    def game_loop(self):
        ...
        while self.is_running:
            ...
            # Automatically mark notifications as read before checking unread count
            self.player.notification_manager.read_notifications()
